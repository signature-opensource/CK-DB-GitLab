-- SetupConfig: {}
--
-- @GitLabAccountId can not be null. This is the key that identifies a GitLab user.
--
...
--
-- When @UserId is not 0, it must match with the one of the @GitLabAccountId otherwise it is an error
-- and an exception is thrown because:
--  - When updating it means that there is a mismatch of UserId/GitLab account in the calling code.
...
--
create procedure CK.sUserGitLabUCL
(
	@ActorId int,
	@UserId int /*input*/output,
	@GitLabAccountId varchar(36), 
...
begin
	...
	if @GitLabAccountId is null throw 50000, 'Argument.NullGitLabAccountId', 1;
	...
	select	@ActualUserId = UserId,
		...
		from CK.tUserGitLab 
		where GitLabAccountId = @GitLabAccountId;
	...
	begin
		...
		begin
			...
			-- Unique constraint on GitLabAccountId will detect any existing UserId/GitLabAccountId clashes.
	...
	begin
		...
		begin
			...
			else if @UserId <> @ActualUserId throw 50000, 'Argument.UserIdAndGitLabIdMismatch', 1;
			...
			update CK.tUserGitLab set 
					LastLoginTime = LastLoginTime 
				where UserId = @ActualUserId and GitLabAccountId = @GitLabAccountId;
	...
	begin
        ...
        begin
		    exec CK.sAuthUserOnLogin 'GitLab', @LastLoginTime, @UserId, @ActualLogin, @Now, @LoginFailureCode output, @LoginFailureReason output;  
            if @ActualLogin = 1 and @LoginFailureCode is null
            begin
			    update CK.tUserGitLab set LastLoginTime = @Now
                    where UserId = @UserId and GitLabAccountId = @GitLabAccountId;
            end
        end
	end
